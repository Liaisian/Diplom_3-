...
class OrderFeedPage(BasePage):
    ...
    def click_to_the_order(self):
        ...
        self.find_element_with_wait(OrderFeedLocators.FIRST_ORDER)

    @allure.step("Проверка открытия модального окна")
    def modal_box_is_open(self):
        if self.find_element_with_wait(OrderFeedLocators.MODAL_ORDER):
            return True

    @allure.step("Получение ID заказа")
    def get_order_id(self):
        self.find_element_with_wait(OrderFeedLocators.LOADING_CHECK_BOX)
        order_id = self.find_element_with_wait(OrderFeedLocators.ORDER_ID).text
        while order_id == '9999':
            order_id = self.find_element_with_wait(OrderFeedLocators.ORDER_ID).text
        return f"#{order_id}"
    ...
    def check_order_id(self, order_id, locator):
        elements = self.find_element_with_wait(locator)
    ...
    def get_total_order_count(self):
        return self.find_element_with_wait(OrderFeedLocators.TOTAL_ORDER_COUNT).text

    @allure.step("Получение количества заказов в день")
    def get_daily_order_count(self):
        return self.find_element_with_wait(OrderFeedLocators.DAILY_ORDER_COUNT).text

    @allure.step("Получение ID заказа в работе")
    def get_id_order_in_the_process(self):
        self.find_element_with_wait(OrderFeedLocators.ORDER_IN_PROCESS)
        return self.find_element_with_wait(OrderFeedLocators.ORDER_IN_PROCESS).text
