...
class FunctionalPage(BasePage):
    ...
    def go_to_order_feed(self):
        self.click_to_element(FunctionalLocators.ORDER_TEXT)
    ...
    def go_to_constructor(self):
        self.click_to_element(FunctionalLocators.CONSTRUCTORS_BUTTON)

    @allure.step("Переход к ингредиенту")
    def go_to_ingredient(self):
        self.click_to_element(FunctionalLocators.INGREDIENT)
        self.find_element_with_wait(FunctionalLocators.INGREDIENT_DETAILS)

    @allure.step("Ожидание деталей ингредиента")
    def wait_details(self):
        if self.find_element_with_wait(FunctionalLocators.INGREDIENT_DETAILS):
            return True

    @allure.step("Закрытие модального окна")
    def close_modal(self):
        self.click_to_element(FunctionalLocators.CLOSE_MODAL)
    ...
    def closing_confirmation(self):
        if self.find_element_with_wait(FunctionalLocators.CLOSE_INGREDIENT_BUTTON):
            return True

    @allure.step("Получение количества ингредиентов")
    def number_of_ingredients(self):
        return self.find_element_with_wait(FunctionalLocators.COUNT_INGREDIENT)

    @allure.step("Создание заказа")
    def make_an_order(self):
        self.click_to_element(FunctionalLocators.CHECKOUT)

    @allure.step("Подтверждение создания заказа")
    def confirmation_of_order(self):
        if self.find_element_with_wait(FunctionalLocators.ORDER_NAME):
            return True
    ...
    def go_to_login_page(self):
        self.find_element_with_wait(FunctionalLocators.ENTER_BUTTON)
        self.click_to_element(FunctionalLocators.ENTER_BUTTON)